...
/**
 * <code>VideoRecorder</code> copies the frames it receives to video. To ensure
 * smooth video at a constant framerate, you should set your application's timer
 * to a new <code>IsoTimer</code>. This class will auto-determine the framerate
 * of the video based on the time difference between the first two frames it
 * receives, although you can manually set the framerate by calling
 * <code>setFps(newFramerate)</code>. Be sure to place this processor *after*
 * any other processors whose effects you want to be included in the output
 * video. You can attach multiple <code>VideoProcessor</code>s to the same
 * <code>ViewPort</code>.
 * 
 * For example, <code>
 ...
 * </code>
 * 
 * will output a video without shadows to <code>file1</code> and a video with
 * shadows to <code>file2</code>
 * 
